...
export default class MapServer{
    ...
    async setAuthentication(strUser, strPwd){
        try {
            let result = await MS.setAuthentication(this._MGMapServerId, strUser, strPwd);
            return result;
        } catch (e) {
            console.error(e);
        }
    ...
    async getAuthenticPassword(){
        try {
            let authenticPassword = await MS.getAuthenticPassword(this._MGMapServerId);
...
MapServer.MapServerType = {
	 ...
	 MAPSERVER_TYPE_TIANDITU_MERCATOR_SATELLITEANNMAP : "TianDiTuMercatorSatelliteAnnMap",
