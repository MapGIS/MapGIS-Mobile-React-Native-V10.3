...
export default class FeatureEdit{
  ...
  async createObjByIGSDoc(strIGServerBaseURL, strDocName, mapID, layerID) {
    try{
        var {FeatureEditId} = await FE.createObj(strIGServerBaseURL, strDocName, mapID, layerID);
        var featureEdit = new FeatureEdit();
        featureEdit._MGFeatureEditId = FeatureEditId;
        return featureEdit;
    }catch(e){
  ...
  async createObjByIGSData(strIGServerBaseURL, strDataURL) {
    try{
        var {FeatureEditId} = await FE.createObj(strIGServerBaseURL, strDataURL);
        var featureEdit = new FeatureEdit();
        featureEdit._MGFeatureEditId = FeatureEditId;
        return featureEdit;
    }catch(e){
  ...
  /**
	...
	{
		try {
            return await FE.update(this._MGFeatureEditId, objID, feature._MGFeatureId);
          } catch (e) {
            console.error(e);
          }
	}
	...
	{
		try {
            return await FE.createCls(this._MGFeatureEditId, clsType, name, geomType, flds._MGFieldsId);
          } catch (e) {
            console.error(e);
          }
	}
