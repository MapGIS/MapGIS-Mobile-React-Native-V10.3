...
export default class SpaAnalysis {
  ...
  async bufferWithEndCapStyle(geom, leftDis, rightDis, endCapStyle, sRefSrc) {
    try {
      let { GeoPolygonsId } = await SPA.buffer(
        this._MGSpaAnalysisId,
        geom._MGGeometryId,
        leftDis,
        rightDis,
        endCapStyle,
        sRefSrc._MGSRefDataId
      );
  ...
  async clipWithType(geom, clipPoly, flag) {
    try {
      let { GeometryId } = await SPA.clip(
        this._MGSpaAnalysisId,
        geom._MGGeometryId,
        clipPoly._MGGeoPolygonId,
        flag
      );
  ...
  async convexHull(geom) {
    try {
      let { GeoPolygonId } = await SPA.convexHull(
        this._MGSpaAnalysisId,
        geom._MGGeometryId
      );
  ...
  async difference(inputGeom, differenceGeom) {
    try {
      let { GeometryId } = await SPA.difference(
        this._MGSpaAnalysisId,
        inputGeom._MGGeometryId,
        differenceGeom._MGGeometryId
      );
      var geometry = new Geometry();
      geometry._MGGeometryId = GeometryId;
      return geometry;
  ...
  async intersection(inputGeom, intersectGeom) {
    try {
      let { GeometryId } = await SPA.intersection(
        this._MGSpaAnalysisId,
        inputGeom._MGGeometryId,
        intersectGeom._MGGeometryId
      );
      var geometry = new Geometry();
      geometry._MGGeometryId = GeometryId;
      return geometry;
  ...
  async symmetricDifference(inputGeom, intersectGeom) {
    try {
      let { GeometryId } = await SPA.symmetricDifference(
        this._MGSpaAnalysisId,
        inputGeom._MGGeometryId,
        intersectGeom._MGGeometryId
      );
  ...
  async mergeGeoPolygons(GeoPolygonArry) {
    try {
      let { GeometryId } = await SPA.merge(
        this._MGSpaAnalysisId,
        GeoPolygonArry
      );
      var geometry = new Geometry();
  ...
  async union(inputGeom, unionGeom) {
    try {
      let { GeometryId } = await SPA.union(
        this._MGSpaAnalysisId,
        inputGeom._MGGeometryId,
        unionGeom._MGGeometryId
      );
      var geometry = new Geometry();
      geometry._MGGeometryId = GeometryId;
      return geometry;
  ...
  async split(geom, line) {
    try {
      let { GeometryId } = await SPA.split(
        this._MGSpaAnalysisId,
        geom._MGGeometryId,
        line._MGGeoLineId
      );
  ...
  async boundary(geom) {
    try {
      let { GeometryId } = await SPA.boundary(
        this._MGSpaAnalysisId,
        geom._MGGeometryId
      );
      var geometry = new Geometry();
